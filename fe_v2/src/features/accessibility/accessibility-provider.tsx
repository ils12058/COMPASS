...
function parseStoredPreferences(raw: string | null): AccessibilityPreferences {
  if (!raw || raw.length > MAX_STORED_LENGTH) return DEFAULT_PREFERENCES;
...
function getClientSnapshot(): AccessibilityPreferences {
  if (preferMemorySnapshot || typeof window === "undefined") {
...
function publishPreferences(preferences: AccessibilityPreferences) {
...
export function AccessibilityPreferencesProvider({ children }: { children: ReactNode }) {
  ...
  const setTextSize = useCallback(
    (textSize: AccessibilityTextSize) => publishPreferences({ ...preferences, textSize }),
    [preferences],
  );
  const setContrast = useCallback(
    (contrast: AccessibilityContrast) => publishPreferences({ ...preferences, contrast }),
    [preferences],
  );
  const setSpacing = useCallback(
    (spacing: AccessibilitySpacing) => publishPreferences({ ...preferences, spacing }),
    [preferences],
  );
  const setUnderlineLinks = useCallback(
    (underlineLinks: boolean) => publishPreferences({ ...preferences, underlineLinks }),
    [preferences],
  );
  const setMotion = useCallback(
    (motion: AccessibilityMotion) => publishPreferences({ ...preferences, motion }),
    [preferences],
  );
  ...
  const value = useMemo<AccessibilityPreferencesContextValue>(
    ...
    [preferences, setTextSize, setContrast, setSpacing, setUnderlineLinks, setMotion],
  );
