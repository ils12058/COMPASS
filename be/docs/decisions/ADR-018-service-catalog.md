...
There is no Service delete endpoint. Disabling only prevents new future workflows from offering the Service; it does not imply cascading deletion or cancellation. No production Service rows are seeded because institutional codes, durations, modes, and provider mappings have not been confirmed.
