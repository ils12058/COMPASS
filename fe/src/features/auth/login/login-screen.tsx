...
export function LoginScreen() {
  ...
  async function submitMfa(method: LoginMfaMethod, code: string) {
    ...
    try {
      ...
      if (!response.data.authenticated) {
        setError("That verification response could not be used. Please try again.");
        return;
      }
      ...
      setError(
        getApiErrorCode(caught) === "mfa_failed"
          ? "That verification response could not be used. Please try again."
  ...
  if (stage === "mandatory_setup" && totpSetup) {
    return (
      <AuthShell
        ...
        description="Your account requires two-step verification before you can sign in."
      >
        <TotpSetup
          setup={totpSetup}
  ...
  if (stage === "recovery_codes") {
    return (
      <AuthShell
        ...
        description="Finish setting up two-step verification before returning to sign in."
      >
        <RecoveryCodesPanel
          codes={recoveryCodes}
          onDone={() => {
            ...
            setStage("login");
            setNotice(
              "Two-step verification is ready. Sign in again to create your COMPASS session.",
            );
          }}
        />
      </AuthShell>
  ...
  if (stage === "mfa") {
    return (
      <AuthShell
        ...
        description="Complete verification to finish signing in to COMPASS."
      >
        <LoginMfaForm
          methods={mfaMethods}
  ...
  return (
    <AuthShell
      ...
      description="Access guidance and counseling services through your University of Camarines Norte account."
    >
      <LoginForm
