...
export function serviceAdminError(
  error: unknown,
  fallback = "We couldn’t complete that service change. Please try again.",
) {
  return getApiErrorMessage(error) ?? fallback;
}

export function formatServiceValue(value: string) {
  return value
    .toLowerCase()
    .replace(/[_-]+/g, " ")
...
export function ServiceActionMessage({
  error,
  message,
}: {
  error?: string | null;
...
export function ServiceListSkeleton() {
  return (
    <div className="space-y-3" aria-live="polite" aria-label="Loading services">
...
export function ServiceQueryError({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="rounded-2xl border border-dashed border-[var(--compass-border-strong)] bg-[var(--compass-surface-subtle)] p-6 text-center">
      <p role="alert" className="text-sm leading-6 text-muted-foreground">
        We couldn’t load the service catalog right now.
      </p>
...
export const serviceSelectClassName =
